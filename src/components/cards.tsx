...
export class CardGroup extends React.PureComponent<React.HTMLProps<HTMLDivElement>&{classes?,sheet?},any>{
    render(){
        const {children, classes, sheet, ...rest} = this.props;
        return <div className={classes['card-group']} {...rest}>
            {children}
        </div>
    }
}
...
export class NumberCard extends React.PureComponent<NumberCardProps&React.HTMLProps<any>&{classes?,sheet?},any>{
    render() {
        const {title, children, classes, sheet, onClick, ...rest} = this.props;
        return <div {...rest} className={classes.card} onClick={onClick}>
